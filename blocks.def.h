...
static const Block blocks[] = {
	...
	{"", "printf %s%s%s%s \"^c#A3B3FF^\" CPU: $(sensors | grep CPU | awk '{print $2}') \"^d^\"",	60,		0},
	{"", "printf %s%s%s%s \"^c#7008E7^\" $($HOME/.local/bin/volume.sh) \"^d^\"",	0,		2},
	{"", "printf %s%s%s%s \"^c#FF2056^\" $($HOME/.local/bin/battery.sh) \"^d^\"",	60,		1},
